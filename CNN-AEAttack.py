...
def oneStepSearch(startX, size, label, sess, grad):
    ...
    gradient_value = sess.run(grad, feed_dict={x: startX.reshape(-1, 6, 8, 1),
                                               y: label.reshape(-1, 2),
...
def AESearch(startFeature, step, size, label, sess, model, grad):
    ...
    while count < step + 1:
        # print("count", count)
        # print(startFeature)
        if np.not_equal(np.argmax(model.predict(startFeature.reshape(1, 6, 8, 1)).reshape(2)),
                        np.argmax(label)):
            return startFeature
...
def advEval(model, size, steps, sess, grad):
    ...
    AML_result = model.evaluate(adv_sample.reshape(adv_sample.shape[0], 6, 8, 1), badDataLabel)
...
targetModel = load_model("CNN_MODEL.h5")
...
x = tf.placeholder(tf.float32, shape=(None, 6, 8, 1))
