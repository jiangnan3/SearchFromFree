...
def oneStepSearch(startX, size, label, sess, grad):
    ...
    gradient_value = sess.run(grad, feed_dict={x: startX.reshape(-1, startX.shape[0]),
                                               y: label.reshape(-1, 2),
                                               keras.backend.learning_phase(): 0})

    gradient_value = gradient_value.reshape(gradient_value.shape[1])
...
def AESearch(startFeature, step, size, label, sess, model, grad):
    ...
    while count < step + 1:
        # print("count", count)
        # print(startFeature)
        if np.not_equal(np.argmax(model.predict(startFeature.reshape(1, startFeature.shape[0])).reshape(2)),
                        np.argmax(label)):
            return startFeature
...
def advEval(model, size, steps, sess, grad):
    ...
    badData = pandas.DataFrame(np.zeros((testDataNum, 48)))
    ...
    AML_result = model.evaluate(adv_sample.reshape(adv_sample.shape[0], adv_sample.shape[1]), badDataLabel)
    print('Afer AML Test loss:', AML_result[0])
    print('After AML Test accuracy:', AML_result[1])
    print("the overall energy consumption is: ", np.sum(np.sum(adv_sample)))
...
targetModel = load_model("FNN_MODEL.h5")
